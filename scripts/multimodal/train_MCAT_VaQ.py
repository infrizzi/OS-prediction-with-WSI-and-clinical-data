...
def main():
    ...
    d_clin = 463                 # output dim clinical encoder
    d_model = 463                # cross-att internal dim
    n_head = 1                   # cross-att number of heads
    ...
    visual_head = RegressionHead(input_dim=d_vis, dropout=0.3) if FUSION_MODE in {"late", "both"} else None
    ...
    load_tensor(model.abmil_vis, ABMIL_CKPT)
    # load_tensor(model.abmil_clin, ABMIL_CKPT)
    ...
    set_requires_grad(model.abmil_vis, False)
    set_requires_grad(model.abmil_clin, True)
    ...
    for name, param in model.named_parameters():
        ...
        # Discriminate between different params
    ...
    param_groups = [
        ...
        {'params': fusion_params, 'lr': LR},       # higher LR -> learning faster
        {'params': head_params, 'lr': LR},           # standard LR (1e-4)        
        {'params': encoder_params, 'lr': 1e-3},      # lower LR for encoders  
    ]
